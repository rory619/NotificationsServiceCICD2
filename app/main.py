...
def list_notifications(limit: int = 10, offset: int = 0, db: Session = Depends(get_db)):
    stmt = (
        select(NotificationsDB).order_by(NotificationsDB.id).limit(limit).offset(offset)
    )
...
@app.put(
    "/api/notifications/{notification_id}",
    response_model=NotificationRead,
    summary="Update an existing notification (full replace)",
)
...
@app.delete(
    "/api/notifications/{notification_id}", status_code=status.HTTP_204_NO_CONTENT
)
